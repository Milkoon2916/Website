...
- **sentences 배열의 길이는 사용자 메시지에 [번호]로 미리 나뉘어 제공되는 문장 개수와
  정확히 같아야 합니다.** 사용자 메시지에 "[1] ...", "[2] ..." 처럼 문장이 이미 번호별로
  나뉘어 주어집니다. 이 구분을 그대로 따르세요 -- 번호를 합치거나, 하나의 번호를 여러
  문장으로 쪼개거나, 순서를 바꾸지 마세요. sentences[i].num은 그 [번호]와 정확히 일치해야
  합니다. (지문 원문 전체도 참고용으로 같이 주어지니 번역·문맥 파악에 활용하세요.)
