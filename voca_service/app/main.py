...
def clean_json(text: str) -> dict:
    ...
    try:
        ...
        # 응답이 중간에 잘렸거나 형식이 깨진 경우 한 번 더 시도: 마지막으로 닫히는 지점까지만 잘라서 파싱
        ...
        raise HTTPException(
            status_code=502,
            detail=f"AI 응답을 JSON으로 해석하지 못했습니다. 응답이 너무 길어 중간에 잘렸을 수 있어요. "
                   f"잠시 후 다시 시도하거나, 문항 수/지문 길이를 줄여보세요. (응답 시작 부분: {snippet})",
        )
